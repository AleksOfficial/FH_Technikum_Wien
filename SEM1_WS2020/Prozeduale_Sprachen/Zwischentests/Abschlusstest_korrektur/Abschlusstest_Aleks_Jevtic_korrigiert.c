...
void set_next_element(hello_world*hello,int points, exam_element * new_element)
{
  ...
  while(element != NULL)
  {
    ...
    prev = element;
    element = element->next;
    //!!! Wenn element == NULL, muss das neue element am vorherigen eingefügt werden!
    if(element== NULL)
    {
      prev->next = new_element;
    }
...
double percentage_pos(hello_world * hello)
{
  ...
  //return total_amount/hello->count_pos*100; OTHER WAY ROUND OMG
  return hello->count_pos/total_amount*100;
}
