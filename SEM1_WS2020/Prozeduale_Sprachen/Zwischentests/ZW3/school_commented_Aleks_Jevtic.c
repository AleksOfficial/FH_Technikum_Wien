...
class *create_class(int pupil_amount, int opcode, school *_school)
{
  ...
  //eine Klasse wird min erstellt, je nach opcode werden die Klassen erzeugt
  //vl hätte man da die Statements wie zb ab malloc bis next in eine eigene Funktion packen können.. too bad! funktioniert so
  class *new_class = malloc(sizeof(class));
  new_class->letter = 'A';
  new_class->number = 1;
  new_class->pupilcounter = pupil_amount; //inital setup, wird bei höherer klassenzahl später korrigiert
  new_class->next = NULL;
  //15-30 Pupils
  if (opcode == 1)
  {
  ...
  else
  {
    ...
    //Korrektur der A-Klasse & der Gesamtanzahl der verteilten Schüler
    new_class->pupilcounter = pupil_amount / (amount_classes + 1);
    distributed_pupils += new_class->pupilcounter;

    //Hier wird von A weg an jede Klasse die übrigen schüler verteilt
...
void update_existing_classes(school *_school)
{
  ...
  if (_school->classlist != NULL)
  {
    //Spezialfall nur 4te Klassen. Generell den Pointer auf NULL setzen hätte man vl in der free_all Funktion packen können... too bad! 
...
{
  //Fügt die Klassen zur Schule hinzu
  int pupil_amount = 0;
...
{
  //Löschfunktion, ändert auch die Gesamtanzahlen pupils und classes der Schule
